from .. import bot, Calls, chat_id
from pyrogram import filters
from ..functions import admin_check, user_input, youtube_stream, video_stream
import asyncio
...
async def pause(client, message):
    if not Calls.is_running:
        return await message.reply("There is Nothing Streaming, What Shall be Paused?")
    admins = await admin_check(client, message)
    if message.from_user.id not in admins:
        return await message.reply("You Dont Have Sufficient Permissions!, Make Sure You Have Manage Video Chats")
...
async def resume(client, message):
    if not Calls.is_running:
        return await message.reply("There is Nothing Streaming, What Shall be Resumed?")
    admins = await admin_check(client, message)
    if message.from_user.id not in admins:
        return await message.reply("You Dont Have Sufficient Permissions!, Make Sure You Have Manage Video Chats")
...
async def media_ended(_, __):
    if que.empty():
        await bot.send_message(chat_id, "No More Videos In Queue!\n\nLeaving Video Chat! xD")
        return await Calls.stop()
    else:
    ...
    try:
        ...
        return await bot.send_photo(chat_id, photo = thumb, caption = f"Started Streaming!\n\n**Video🎥** : **__{title}__**\n**Chat : {ctitle}**")
    except Exception as e:
